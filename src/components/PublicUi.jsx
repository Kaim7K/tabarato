...
export function SectionHeader({ eyebrow = "", title, description = "", children = null }) {
  return (
    <section className="bg-white border-b border-[#111111]/10 shadow-[0_1px_0_rgba(17,17,17,0.02)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        {eyebrow && <p className="text-[#FF6B35] text-xs font-bold uppercase tracking-[0.08em] mb-2">{eyebrow}</p>}
    ...
    <div role="status" aria-live="polite">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {Array.from({ length: 8 }, (_, index) => (
          <div key={index} className="bg-white rounded-lg overflow-hidden border border-[#111111]/8 shadow-[0_4px_18px_rgba(17,17,17,0.04)] animate-pulse">
...
export function EmptyState({ icon: Icon = PackageSearch, title, description = "", action = null }) {
  return (
    <div className="bg-white border border-[#111111]/8 rounded-lg text-center py-14 sm:py-16 px-5 shadow-[0_8px_28px_rgba(17,17,17,0.05)]">
      <div className="w-14 h-14 mx-auto mb-4 rounded-lg bg-[#F4F5F6] flex items-center justify-center"><Icon className="w-7 h-7 text-[#111111]/25" aria-hidden="true" /></div>
  ...
  return (
    <button
      ...
      className={`min-h-10 px-4 py-2 text-sm font-medium rounded-md whitespace-nowrap transition focus:outline-none focus:ring-2 focus:ring-[#FF6B35]/35 ${
        active ? "bg-[#111111] text-white shadow-[0_4px_12px_rgba(17,17,17,0.14)]" : "bg-white text-[#111111]/65 hover:text-[#111111] hover:border-[#111111]/20 border border-[#111111]/10"
      }`}
      {...props}
    >
      {children}
...
export function SectionTitle({ eyebrow = "", title, action = null }) {
  return (
    <div className="flex items-end justify-between gap-4 mb-5">
      <div>
        {eyebrow && <p className="text-[#FF6B35] text-xs font-bold uppercase tracking-[0.08em] mb-1">{eyebrow}</p>}
