    ...
    <div className="bg-[#F5F2EB] min-h-screen">
      ...
      <section className="sticky top-16 sm:top-20 z-30 bg-[#F5F2EB]/85 backdrop-blur-md border-b border-[#111111]/8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-wrap gap-2">
          {priceFilters.map((f) => (
            <button key={f.key} onClick={() => setPriceFilter(f.key)} className={`px-4 py-2 text-sm font-medium rounded-full transition ${priceFilter === f.key ? "bg-[#111111] text-white" : "bg-white text-[#111111]/60 border border-[#111111]/8"}`}>
              {f.label}
            </button>
          ))}
          {platforms.length > 0 && (
