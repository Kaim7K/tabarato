        ...
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-16">
            ...
            <div className="lg:sticky lg:top-28">
              ...
              <div className="bg-white rounded-2xl p-6 shadow-[0_20px_50px_rgba(0,0,0,0.04)]">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <p className="text-[#111111]/40 text-xs uppercase tracking-wide mb-1">Pontuação</p>
                    <p className="text-3xl font-bold text-[#111111]" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                      {offer.score}<span className="text-[#111111]/30 text-lg">/100</span>
                    </p>
                  </div>
                  <span className="px-3 py-1.5 bg-[#168A55]/10 text-[#168A55] text-xs font-semibold rounded-full">
                    {offer.score >= 80 ? "Recomendada" : offer.score >= 65 ? "Em análise" : "Rascunho"}
                  </span>
                </div>
                <div className="w-full h-2 bg-[#F5F2EB] rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full ${offer.score >= 80 ? "bg-[#168A55]" : offer.score >= 65 ? "bg-[#FF6B35]" : "bg-[#111111]/30"}`}
          ...
          <div className="lg:col-span-6">
                ...
                <ul className="space-y-3">
                  {reasons.map((reason, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      <div className="w-5 h-5 rounded-full bg-[#168A55]/10 flex items-center justify-center shrink-0 mt-0.5">
                        <Check className="w-3 h-3 text-[#168A55]" />
                      </div>
            ...
            <a
              ...
              onClick={() => base44.entities.Offer.update(offer.id, { clicks: (offer.clicks || 0) + 1 }).catch(() => {})}
              className="w-full flex items-center justify-center gap-2 px-8 py-5 bg-[#FF6B35] hover:bg-[#D95426] text-white font-bold rounded-full transition shadow-lg text-lg mb-3"
            >
              Ver oferta
            ...
            <div className="flex gap-3 mb-4">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 px-6 py-3 rounded-full font-semibold transition text-sm ${
                  isFav ? "bg-[#FF6B35] text-white" : "bg-white text-[#111111] border border-[#111111]/10 hover:bg-[#F5F2EB]"
                }`}
              >
