...
export default function Compare() {
  ...
  const { compareIds, toggleCompare } = useOfferTools();
    ...
    <PageShell>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        {loading ? <LoadingState /> : !offers.length ? (
          <EmptyState icon={Scale} title="Nenhuma oferta para comparar." description="Use o botão de comparação nos cards para adicionar até três produtos." action={<Link to="/" className="inline-flex min-h-11 items-center px-5 bg-[#FF6B35] text-white font-semibold rounded-md">Explorar ofertas</Link>} />
        ) : (
          <div className="overflow-x-auto rounded-lg border border-[#111111]/10 bg-white shadow-[0_6px_24px_rgba(17,17,17,0.05)]">
            <div className="grid min-w-[720px]" style={{ gridTemplateColumns: `11rem repeat(${offers.length}, minmax(12rem, 1fr))` }}>
              <CompareLabel />
              {offers.map((offer) => <ProductHeader key={offer.id} offer={offer} remove={() => toggleCompare(offer.id)} />)}
...
function ProductHeader({ offer, remove }) {
  return (
    <div className="relative p-4 border-b border-r last:border-r-0 border-[#111111]/10">
      <button type="button" onClick={remove} className="absolute top-2 right-2 w-9 h-9 flex items-center justify-center rounded-full bg-white border border-[#111111]/10" aria-label={`Remover ${offer.name}`}><X className="w-4 h-4" /></button>
