...
export async function listOffers({ search = "", status = "", category = "" } = {}) {
  ...
  const result = await query(sql, params).catch(async (error) => {
    ...
    return query(`SELECT
      ...
      NULL AS telegram_next_retry_at, NULL AS telegram_last_error_code, NULL AS campaign_name,
      0 AS priority, '{}'::jsonb AS intelligence_evidence, clicks AS real_clicks,
